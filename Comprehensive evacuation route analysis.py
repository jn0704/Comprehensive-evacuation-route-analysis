...
shelter_ds = ogr.Open('shelter location point lyer.shp')
...
"""
The second module finds the shortest path among the paths that have shorter hazard length than
the original shortest path between population point and shelter point.
If there is no other path except the original shortest path, the algorithm saves only the path.
"""
## Create a empty dataframe
...
for pop in pop_point :
    ...
    for shelter in shel_point :
        ...
        shelter_cap = shelter[2]
        ## calculate the shortest path
        shortest_path = nx.dijkstra_path(G, pop_node, shelter_node, weight='HAZARD')
        shortest_hazard = nx.shortest_path_length(G, pop_node, shelter_node, weight='HAZARD')
        shortest_len = 0
        for i in range(0, len(shortest_path)-1) :
            shortest_len += G[shortest_path[i]][shortest_path[i+1]]['LEN']
        row = [pop_id, shelter_id, pop_node, shelter_node,
               pop_num, shelter_cap, shortest_len, shortest_hazard]
...
hazard_df = pd.read_json('.json')
comparing_shortest_df = pd.read_json('.json')
...
total_pop = shortest_df['POP'].sum()
total_cap = shortest_df['CAP'].sum()
## get column index
...
"""
Second, we analyze the matrix to minimize the road hazard.
"""
## Assings the matrix considering only the shortest path
## Convert dataframe for analysis
hazard_df['ASSIGN'] = 0
pop_df = hazard_df.drop_duplicates('POP_ID', keep='first')
shel_df = hazard_df.drop_duplicates('SHELTER_ID', keep='first')
pop_df.reset_index(drop=True, inplace=True)
shel_df.reset_index(drop=True, inplace=True)
hazard_df['POP_COPY'] = hazard_df['POP']
hazard_df['CAP_COPY'] = hazard_df['CAP']
total_pop = hazard_df['POP'].sum()
total_cap = hazard_df['CAP'].sum()
## get column index
pop_id_index = list(hazard_df.columns).index('POP_ID')
shel_id_index = list(hazard_df.columns).index('SHELTER_ID')
pop_index = list(hazard_df.columns).index('POP')
cap_index = list(hazard_df.columns).index('CAP')
assign_index = list(hazard_df.columns).index('ASSIGN')
pop_copy_index = list(hazard_df.columns).index('POP_COPY')
cap_copy_index = list(hazard_df.columns).index('CAP_COPY')

## Create shapefile
driver = ogr.GetDriverByName('ESRI Shapefile')
data_source = driver.CreateDataSource('.shp')
create_lyr = data_source.CreateLayer('comprehensive', road_lyr.GetSpatialRef(), ogr.wkbMultiLineString)
data_source.Destroy()

## Create shortest path as linestring
## read shapefile
path_ds = ogr.Open('.shp', 1)
path_lyr = path_ds.GetLayer('comprehensive')
path_defn = path_lyr.GetLayerDefn()
print(path_lyr.GetGeomType() == ogr.wkbLineString)
...
while True :
    print('--------------------')
    ## find the shortest path
    min_cost = hazard_df[(hazard_df['POP_COPY']>0) & (hazard_df['CAP_COPY']>0)]['LEN'].min() # select the population that has the shortest path
    print('cost: ' + str(min_cost))
    if math.isnan(min_cost) == True :
        print('assigning is finish!')
        print('but, some pops are not assigned!')
        additional_df = hazard_df.loc[hazard_df["POP_COPY"] == -1]
        break
    index = hazard_df.loc[(hazard_df['POP_COPY']>0) & (hazard_df['CAP_COPY']>0) &
                          (hazard_df['LEN'] == min_cost)].index.values[0] # get row index
    print('index: ' + str(index))
    
    ## get values
    pop_id = hazard_df.iloc[index]['POP_ID']
    hazard = hazard_df.iloc[index]['HAZARD']
    if hazard > comparing_shortest_df.loc[comparing_shortest_df["POP_ID"]==pop_id]["HAZARD"].min() :
        hazard_df.iloc[index, pop_copy_index] = -1
        print('pass')
        continue
    shel_id = hazard_df.iloc[index]['SHELTER_ID']
    print('shel_id: ' + str(shel_id))
    pop = float(hazard_df.iloc[index]['POP_COPY'])
    cap = float(hazard_df.iloc[index]['CAP_COPY'])
    pop_node = int(hazard_df.iloc[index]['POP_NODE'])
    shel_node = int(hazard_df.iloc[index]['SHELTER_NODE'])
    length = hazard_df.iloc[index]['LEN']
    shortest_path = nx.dijkstra_path(G, pop_node, shel_node, weight='HAZARD')

    
    ## assigning
    if cap >= pop :
        hazard_df.iloc[hazard_df[hazard_df['POP_ID']==pop_id].index.values, pop_copy_index] = 0
        hazard_df.iloc[hazard_df[hazard_df['SHELTER_ID']==shel_id].index.values, cap_copy_index] = cap - pop
        hazard_df.iloc[index, assign_index] = pop
        ## draw routes
        geom = ogr.Geometry(ogr.wkbLineString)
        ## Create multilinestring
        for i in range(0,len(shortest_path)):
...
#         print(feat.geometry())
        ...
        hazard_df.iloc[hazard_df[hazard_df['POP_ID']==pop_id].index.values, pop_copy_index] = pop - cap
        hazard_df.iloc[hazard_df[hazard_df['SHELTER_ID']==shel_id].index.values, cap_copy_index] = 0
        hazard_df.iloc[index, assign_index] = cap
                ## draw routes
        geom = ogr.Geometry(ogr.wkbLineString)
        ## Create multilinestring
        for i in range(0,len(shortest_path)):
...
#         print(feat.geometry())
    ...
    print('total pop: ' + str(total_pop))
    print('total cap: ' + str(total_cap))
    if hazard_df['POP_COPY'].sum() <= 0 :
        print('assigning is finish!')
        break
    if hazard_df['CAP_COPY'].sum() <= 0 :
        print('capacity issue!')
        break
...
comprehensive_df = hazard_df.loc[hazard_df["ASSIGN"] > 0]
comprehensive_df.to_json('.json')
comprehensive_df.to_csv('.csv')

additional_df.to_json('.json')
additional_df.to_csv('.csv')

## load adatadrames
additional_df = pd.read_json('.json')
comprehensive_df = pd.read_json('.json')
hazard_df = pd.read_json('.json')
shortest_df = pd.read_json('.json')

"""
In this section, we re-assign populations that are not assigned before process.
First, we identify the populations that are not in the list of the result.
And second, we reassign the populations.
In that case, we use the previous shelter information.
We find the shortest path that has shorter hazards than the shortest path.
If not, we find the shortest path among the remained routes.
"""
## find the pops not in the dataframe
assigned_pop = comprehensive_df["POP_ID"].values.tolist()

## find the missing pops
def find_missing(lst) :
    return [i for x, y in zip(lst, lst[1:])  
        for i in range(x + 1, y) if y - x > 1] 
missing_pop = find_missing(assigned_pop)

## create new dataframe for assinging
# Hazard dataframe
missing_hazard_df = hazard_df[hazard_df['POP_ID'].isin(missing_pop)][['POP_ID', 'SHELTER_ID', 'POP_NODE', 'SHELTER_NODE', 'POP',
                                                'CAP', 'LEN', 'HAZARD', 'CAP_COPY']]
missing_hazard_df.columns = ['POP_ID', 'SHELTER_ID', 'POP_NODE', 'SHELTER_NODE', 'POP', 'CAP', 'LEN', 'HAZARD', 'ASS_CAP']
missing_hazard_df.reset_index(drop=True, inplace=True)
# Shortest dataframe
missing_shortest_df = shortest_df[hazard_df['POP_ID'].isin(missing_pop)][['LEN', 'HAZARD']]
missing_shortest_df.columns = ['S_LEN', 'S_HAZARD']
missing_shortest_df.reset_index(drop=True, inplace=True)
#join two dataframes
missing_df = missing_hazard_df.join(missing_shortest_df, how='left')
missing_df['POP_COPY'] = missing_df['POP']
missing_df['CAP_COPY'] = missing_df['ASS_CAP']
missing_df['ASSIGN'] = 0

## get column index
pop_id_index = list(missing_df.columns).index('POP_ID')
shel_id_index = list(missing_df.columns).index('SHELTER_ID')
pop_index = list(missing_df.columns).index('POP')
cap_index = list(missing_df.columns).index('CAP')
assign_index = list(missing_df.columns).index('ASSIGN')
pop_copy_index = list(missing_df.columns).index('POP_COPY')
cap_copy_index = list(missing_df.columns).index('CAP_COPY')

## Create shapefile
driver = ogr.GetDriverByName('ESRI Shapefile')
data_source = driver.CreateDataSource('.shp')
create_lyr = data_source.CreateLayer('additional', road_lyr.GetSpatialRef(), ogr.wkbMultiLineString)
data_source.Destroy()

## Create shortest path as linestring
## read shapefile
path_ds = ogr.Open('.shp', 1)
path_lyr = path_ds.GetLayer('additional')
path_defn = path_lyr.GetLayerDefn()
print(path_lyr.GetGeomType() == ogr.wkbLineString)
...
while True :
    print('--------------------')
    ## find the shortest path
    min_cost = missing_df[(missing_df['POP_COPY']>0) & (missing_df['CAP_COPY']>0)]['LEN'].min() # select the population that has the shortest path
    index = missing_df.loc[(missing_df['POP_COPY']>0) & (missing_df['CAP_COPY']>0) & 
                           (missing_df['LEN'] == min_cost)].index.values[0]
    pop_id = missing_df.iloc[index]['POP_ID']
    print('pop_id: ' + str(pop_id))
    shel_id = missing_df.iloc[index]['SHELTER_ID']
    print('shel_id: ' + str(shel_id))
    hazard = missing_df.iloc[index]['HAZARD']
    scost = missing_df.iloc[index]['S_LEN']
    shazard = missing_df.iloc[index]['S_HAZARD']
    if hazard > shazard :
        print('longer hazard')
        missing_df.iloc[index, pop_copy_index] = -1
        continue
    else :
        pop = float(missing_df.iloc[index]['POP_COPY'])
        cap = float(missing_df.iloc[index]['CAP_COPY'])
        pop_node = int(missing_df.iloc[index]['POP_NODE'])
        shel_node = int(missing_df.iloc[index]['SHELTER_NODE'])
        shortest_path = nx.dijkstra_path(G, pop_node, shel_node, weight='HAZARD')
        
            ## assigning
        if cap >= pop :
            missing_df.iloc[missing_df[missing_df['POP_ID']==pop_id].index.values, pop_copy_index] = 0
            missing_df.iloc[missing_df[missing_df['SHELTER_ID']==shel_id].index.values, cap_copy_index] = cap - pop
            missing_df.iloc[index, assign_index] = pop
            ## draw routes
            geom = ogr.Geometry(ogr.wkbLineString)
            ## Create multilinestring
            for i in range(0,len(shortest_path)):
    ...
    #         print(feat.geometry())
            ...
            missing_df.iloc[missing_df[missing_df['POP_ID']==pop_id].index.values, pop_copy_index] = pop - cap
            missing_df.iloc[missing_df[missing_df['SHELTER_ID']==shel_id].index.values, cap_copy_index] = 0
            missing_df.iloc[index, assign_index] = cap
                    ## draw routes
            geom = ogr.Geometry(ogr.wkbLineString)
            ## Create multilinestring
            for i in range(0,len(shortest_path)):
    ...
    #         print(feat.geometry())
            ...
            num += 1
            print(num)
        if missing_df['POP_COPY'].sum() <= 0 :
            print('assigning is finish!')
            break
        if missing_df['CAP_COPY'].sum() <= 0 :
            print('capacity issue!')
            break
...
path_ds.Destroy()
## save the results
missing_df.to_json('.json')
missing_df.to_csv('.csv')

Missing = missing_df.loc[missing_df["ASSIGN"] > 0]
Missing.to_json('.json')
Missing.to_csv('.csv')
